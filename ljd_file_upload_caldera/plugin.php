<?php
/**
 * Plugin Name: Caldera Forms - File Upload By Lee Jackson
 * Plugin URI:  
 * Description: Custom Field ready file uploader. Using boiler template by Desertsnowman. (Thank you!)
 * Version:     0.0.1
 * Author:      Lee Jackson
 * Author URI:  
 * License:     GPL-2.0+
...
add_filter('caldera_forms_get_field_types', 'ljd_upload_field_register_function');
 ...
 * @return array $fields
 */
function ljd_upload_field_register_function($fields){
	...
	$fields['my_custom_field_type'] = array(
		"field"				=>	__("Custom File Uploader", 'my-plugin-slug'),
		...
		"description" 		=>	__('Custom field compatiable uploader','my-plugin-slug'), 	// description explains what the field is for
		"viewer"			=>	'ljd_uploader_viewer_function', // array($this, 'viewer_function')		// viewer function is used to processes the stored value for display purposes. i.e if saving a saving a post ID the viewer whould get the post title to display
		"handler"			=>	'ljd_uploader_handler_function', // array($this, 'handler_function')		// handler function is used to processes the submitted value before storage. Like a file uploader to store the saved URL
 ...
 */
function ljd_uploader_viewer_function($value, $field ,$form){
 ...
 */
function ljd_uploader_handler_function($value, $field ,$form){
